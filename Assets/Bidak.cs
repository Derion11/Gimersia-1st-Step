...
public class Bidak : MonoBehaviour
{
    ...
    public gridPapan papan;      // drag object Board yang punya script gridPapan

    [Header("Posisi awal (0 = kotak pertama)")]
    public int nomorSaatIni = 0;

    [Header("Gerak satu-per-satu")]
    public bool pakaiGerakHalus = true;
    public float durasiPerLangkah = 0.15f; // detik per kotak
    ...
    public void StartGame()
    {
        ...
        if (papan == null)
        {
            Debug.LogWarning("Bidak: field 'papan' belum di-assign. Drag object Board ke sini.");
            return;
        }
    ...
    public void JalanSatuLangkah()
    {
        ...
        {
            Debug.Log("Bidak sudah di kotak terakhir.");
            return;
        }
    ...
    /// <summary>Korutin: melangkah satu kotak demi satu kotak sampai targetIndex (0-based).</summary>
    private IEnumerator GerakStepByStep(int targetIndex)
    {
        ...
        while (nomorSaatIni < targetIndex)
        {
            ...
            {
                float t = 0f;
                float dur = Mathf.Max(0.0001f, durasiPerLangkah);
                while (t < 1f)
                {
                    t += Time.deltaTime / dur;
                    transform.position = Vector3.Lerp(start, tujuan, t);
                    yield return null;
                }
            }
            else
